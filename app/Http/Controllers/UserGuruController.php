...
class UserGuruController extends Controller
{
    ...
    {
        $user_id = guru::where('user_id', \Auth::user()->id)->first();
        $data['data'] = transaksi::where('status', 'Belum Komfirmasi')->orWhere('status', 'komfirmasi')->where('user_id', $user_id->id)->where('level', 'guru')->get();
    ...
    {
        $user_id = guru::where('user_id', \Auth::user()->id)->first();
        $data['data'] = transaksi::where('user_id', $user_id->id)->where('status', 'kembali')->where('level', 'guru')->get();
